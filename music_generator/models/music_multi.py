...
none (vanilla), continuous_concat
...
class MusicTransformerMulti(torch.nn.Module):
    ...
    def forward(self, x, condition):
        ...
        # embed input to (B, seq, self.embedding_dim - self.d_condition)
        x = self.embedding(x)  
        x *= math.sqrt(self.embedding_dim-self.d_condition)

        if self.d_condition > 0:
            # changed
            condition = self.fc_condition(condition)  # shape (B, seq, self.d_condition)
            x = torch.cat([x, condition], dim=-1)  # concatenate : (B, seq, self.embdding_dim) 
