...
class Runner:
    def __init__(self):
        ...
        train_feats, test_feats = preprocess_features(
            # changed
            "../data/DEAM/features/full_dataset_features_summarized.csv", conditional=conditional,
    ...
    def init_model(self):
        ...
        if args.restart_dir:
            ...
            print("model is loaded from " + args.restart_dir)
        else:
            ...
            for input_, condition, target in self.train_loader:
                ...
                if (self.train_step % args.gen_step == 0) and self.train_step > 0 and not args.regression:
                    ...
                    with torch.no_grad():
                        ...
                        if args.conditioning == "none":
                            ...
                            # shape (B, 1, 2) -> (B, L, 2) let's just repeat
                            continuous_conditions = continuous_cond.expand((-1, max_input_len, -1))
                            
                        generate(self.model, self.maps, self.device, self.gen_dir, args.conditioning, 
                            debug=args.debug, verbose=False, amp=self.amp,
                ...
                if (self.train_step % args.log_step == 0):
                    ...
                    if n_elements_total > 0:
                        ...
                        if not args.debug:  
                            # Save model
                            model_fp = os.path.join(args.work_dir, 'model.pt') # save model.pt, optimizer.pt, scalar.pt, stats.pt at ../output
